...
void Pattern::logic(int stage_speed)
{
    ...
    {
        current_sprite++;
        if(player_raping!=NULL)
        {
            if(current_sprite>=bullet->spritesRaping())
            {
                current_sprite=0;
            }
        }else if(is_hit)
    ...
    if(player_raping!=NULL)
    {
        this->x=player_raping->x;
        this->y=player_raping->y;
    }
}
...
void Pattern::render()
{
    ...
    if(player_raping!=NULL)
    {
        image=bullet->getRapeImage(current_sprite);
    }
    else if(is_hit)
    {
