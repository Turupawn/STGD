...
void Character::loadBulletsXML()
{
            ...
            bullet_node=bullet_node->NextSibling("Bullet"))
    {
        ...
        TiXmlNode* raping_node=bullet_node->FirstChild("Raping");
        vector<Image*>sprites_raping;
        if(raping_node!=NULL)
        {
            for(TiXmlNode* sprite_node=raping_node->FirstChild("Sprite");
                    sprite_node!=NULL;
                    sprite_node=sprite_node->NextSibling("Sprite"))
            {
                sprites_raping.push_back(painter->getTexture(directory+"sprites/"+sprite_node->ToElement()->Attribute("path")));
            }
        }

        bullets[node_name]=new Bullet(sonido,painter,receiver,node_name,sprites_temp,sprites_onhit_temp,sprites_raping,hitboxes_temp,damage);
    }
}
...
//Loading file
            ...
            pattern_type=pattern_type->NextSibling("Type"))
    {
                ...
                pattern_node=pattern_node->NextSibling("Pattern"))
        {
            ...
            bool rapes=false;
            if(pattern_node->ToElement()->Attribute("rapes"))
                rapes=strcmp(pattern_node->ToElement()->Attribute("rapes"),"yes")==0;
            ...
            if(pattern_node->FirstChild("Modifier")!=NULL)
            {
                        ...
                        pattern_modifier_node=pattern_modifier_node->NextSibling("Modifier"))
                {
                    ...
                    if(pattern_modifier_node->ToElement()->Attribute("rapes")!=NULL)
                    {
                        std::string value=pattern_modifier_node->ToElement()->Attribute("rapes");
                        temp_modifiers->push_back(new Modifier("rapes",value));
                    }
